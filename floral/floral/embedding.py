...
class LoraEmbeddingExperts(nn.Module):
    ...
    def forward(self, x, probs):
        outputs = torch.stack([
            self.weight_out[c](self.weight_in[c](x)) for c in range(self.num_loras)
        ])
